...
import static com.e.cursorloader.MainActivity.viewAllMessage;
...
public class ReadMmsSms {
    ...
    static String grading[] = {"일반", "White", "Silver", "실버", "Gold", "골드", "VIP", "VVIP", "VVIP"};
    ...
    class setupAsyncTask extends AsyncTask<Obj_Setup, Integer, Long> {
        ...
        protected void onProgressUpdate(Integer... values) {
            tv_unique.setText((values[0]+1)+" 번째 메시지");
            if (values[0]%30 == 0) {
                viewAllMessage();
                lv_sms.setSelection(lv_sms.getCount()-1);
            }
        ...
        protected void onPreExecute() {
            Toast.makeText(context, "폰(기기)의 메시지 수집 시작", Toast.LENGTH_SHORT).show();
            super.onPreExecute();
        }
        @Override
                ...
                if (one.getTable().equals("sms")) {
                    if (curSMS.moveToNext()) {
                        ...
                        if (!body.trim().isEmpty()) {
                            ...
                            if (date < 10000000000L) date = date * 1000;
                            if (body.contains("통신요금") && (nowdate-date < 8035200000L) && !pay) {
                                if (body.contains("납부")) {
                                    paycount++;
                                    if (paycount == 3)
                                        pay = true;
                            ...
                            values.put("chosung", smsHelper.stringToChosungs(name));
                            values.put("phone", phone);
                            values.put("date", date);
                            values.put("body", body);
                            long newRowId = database.insert(tbl_sms, null, values);
                            if (newRowId == -1) {
                                // 데이터 입력 실패의 경우 처리할 코드
                                final long finalDate = date;
                                MainActivity.mHandler.postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(context, name + "/" + finalDate + " / 데이터 입력 실패", Toast.LENGTH_SHORT).show();
                                    }
                                }, 10);
                            }
                        }
        ...
        protected void onPostExecute(Long result) {
            curSMS.close();
            curMMS.close();
            Toast.makeText(context, "메시지 수집 작업이 끝났습니다.", Toast.LENGTH_SHORT).show();
            viewAllMessage();
